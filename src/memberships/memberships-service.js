...
export const addUserToPlan = async ({ pid, uid, role }) => {
	console.log(pid, uid, role);
	const response = await api.post(`${BASE_API_URL}/memberships/${pid}/${uid}`, {
		role: role,
	});
	return response.data;
};
...
export const updateUserForPlan = async ({ pid, uid, role }) => {
	const response = await api.put(`${BASE_API_URL}/memberships/${pid}/${uid}`, {
		role: role,
	});
	return response.data;
};
...
export const removeUserFromPlan = async ({ pid, uid }) => {
	const response = await api.delete(
		`${BASE_API_URL}/memberships/${pid}/${uid}`
	);
	return response.data;
};
